...
for ii in range(5):
    ...
    tmp=hp.read_map(fname,field=[1,2])
    sigma_e1_per_pix = np.std(tmp[0][mask.astype(bool)])
    sigma_e2_per_pix = np.std(tmp[1][mask.astype(bool)])
    
    shape_noise[ii] = [np.zeros(len(mask)), np.zeros(len(mask))]
    
    shape_noise[ii][0][mask.astype(bool)]=np.random.normal(scale=sigma_e1_per_pix,size=int(sum(mask)))
    shape_noise[ii][1][mask.astype(bool)]=np.random.normal(scale=sigma_e2_per_pix,size=int(sum(mask)))
...
fname=savedir+f'data-noisy-nmt-clkk{vd_tag}-{nside}-binning-20-2000-shapeN-Gaussian.txt'
